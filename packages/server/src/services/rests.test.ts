...
describe('computeHitDiceRestore', () => {
  ...
  it('restores a minimum of 1 even when half rounds down to 0', () => {
    const { restoredCount } = computeHitDiceRestore([{ dieType: 'd8', maxForType: 1 }], { d8: 0 });
  ...
  it('produces no restoration for a character with no classes/hit dice', () => {
    const { restoredCount } = computeHitDiceRestore([], {});
    expect(restoredCount).toBe(0);
  });
});
