...
import { createAssetFieldsSchema, updateAssetSchema, updateAssetVisibilitySchema } from '../schemas/assets.js';
...
assetsRouter.patch('/:id/visibility', async (req, res) => {
  const { visibleToPlayers } = updateAssetVisibilitySchema.parse(req.body);
  const asset = await assetsService.updateAssetVisibility(pool, req.user!.id, (req.params.id as string), visibleToPlayers);
  res.json({ asset });
});
