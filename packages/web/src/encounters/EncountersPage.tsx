    ...
    <div className="px-4 sm:px-6 py-6 max-w-6xl mx-auto">
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          */}
          {openTabIds.length > 1 && (
            <div className="flex gap-1 border-b border-stone-800 mb-4 overflow-x-auto" role="tablist" aria-label="Open encounters">
              {openTabIds.map((id) => {
          ...
          {openTabIds.length === 0 ? (
            <EmptyState message="Select or create an encounter to view its tracker." />
          ) : (
            openTabIds.map((id) => {
              const enc = encountersById.get(id);
              if (!enc) return null;
